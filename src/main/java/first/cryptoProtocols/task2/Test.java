...
public class Test {
    ...
    public void sign() throws IOException {
        ...
        for (int i = 0; i < this.text.length(); i++) {
            ...
            textAsDigits = textAsDigits.concat(String.valueOf(tmp)).concat("333");
        }

        String secretTextAsDigits = "";
        for (int i = 0; i < this.secretText.length(); i++) {
            long tmp = alph.get(secretText.charAt(i));
            secretTextAsDigits = secretTextAsDigits.concat(String.valueOf(tmp)).concat("333");
        }
        ...
        try (BufferedWriter bufferedWriter = new BufferedWriter(new FileWriter(SIGN_FILE_PATH))) {
            ...
            M = secretTextAsDigits;
            Ms = textAsDigits;

            if (!p.subtract(BigInteger.ONE).gcd(new BigInteger(M)).equals(BigInteger.ONE)||
                    (!p.subtract(BigInteger.ONE).gcd(new BigInteger(Ms)).equals(BigInteger.ONE)))
                sign();
            ...
            BigInteger Y = new BigInteger(Ms)
    ...
    public void checkSign() throws Exception {
        ...
        for (int i = 0; i < sign.size(); i += 2) {
            ...
            for (int k = 0; k < this.text.length(); k++) {
                long tmp = alph.get(text.charAt(k));
                textAsDigits = textAsDigits.concat(String.valueOf(tmp)).concat("333");
            }
            Ms = textAsDigits;
            j++;
            ...
            BigInteger right = g.modPow(new BigInteger(Ms), p);
    ...
    public void decryptSecretText() throws IOException {
        ...
        for (int i = 0; i < sign.size(); i += 2) {
            ...
            BigInteger YModInverse = Y.modInverse(p.subtract(ONE));
            BigInteger tmp = new BigInteger(Ms).subtract(r.multiply(X)).multiply(YModInverse).mod(p.subtract(ONE));
            String res = tmp.toString();
            String[] symbols = res.split("333");
            for (String symbol : symbols) {
                result = result.concat(String.valueOf(alphRevert.get(Long.parseLong(symbol))));
                System.out.print(alphRevert.get(Long.parseLong(symbol)));
            }
        }
