...
import static first.cryptoProtocols.task4_2.Protocol.BASE_PATH;
import static first.cryptoProtocols.task4_2.Protocol.DECK_PATH;

public class Help {

    private static Map<Character, Integer> alph = new TreeMap<>();
    private static final String FILES_PATH = "/Users/aleksandr/5 курс 1 семестр/Code/src/main/java/first.cryptoProtocols/task4_2/files/";
...
//            "пики");
    ...
    public static void writeCard(List<String> list, String file) throws IOException {
    ...
    public static BigInteger generatePrime() {
        ...
        int bitLength = 100 + random.nextInt(200);
    ...
    public static void generateDeck() throws IOException {
        int k = 0, j = 0;
        for (int i = 0; i < 52; i++) {
            FileWriter fw = new FileWriter(DECK_PATH + (i + 1) + ".txt");
            String string = card.get(k).concat(mast.get(j)).concat(String.valueOf(((int) ((Math.random() * 100)))));
            fw.write(string);
            fw.close();
            fw = new FileWriter(BASE_PATH + "copy/" + (i + 1) + ".txt");
            fw.write(string);
            if (i % 4 == 0 && i!=0) {
                k = (k + 1);
            }
